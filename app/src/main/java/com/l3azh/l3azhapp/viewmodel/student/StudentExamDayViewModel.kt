...
class StudentExamDayViewModel(application: Application): AndroidViewModel(application) {
    @Inject
    ...
    var listExamDay: MutableLiveData<List<GetExamDayItemResData>> = MutableLiveData()
    ...
    fun getListExamDay(
        ...
        try{
            val response = repository.getListExamDay(bearerToken, semesterSchoolYear,codeOfStudent)
            if(response.isSuccessful){
                listExamDay.postValue(response.body()!!.data)
